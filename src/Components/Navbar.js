...
import "./Navbar.css"; // You may not need this if you use Tailwind classes directly
  ...
  return (
    <header
      className={`fixed top-0 z-50 w-full ${
        reachedProjectSection ? "bg-black" : scrollPosition > 0 ? "bg-orange-500" : "bg-gray-200"
      } text-white backdrop-filter backdrop-blur-lg transition-all duration-500`}
    >
      <div className="container flex flex-wrap items-center justify-between p-5 mx-auto md:flex-row">
        <Link
          ...
          className={`font-bold italic ${
            reachedProjectSection || scrollPosition > 0 ? "text-white" : "text-black"
          } md:mb-0 text-3xl md:text-4xl`} // Adjust font size here
          style={{
            transform: isTilted ? "rotate(-9deg)" : "rotate(0deg)",
        ...
        <div className="block md:hidden">
          <button
            ...
            className="flex items-center px-3 py-2 text-black border rounded hover:text-white hover:border-white"
          >
            <svg
        ...
        >
          <div className="text-base md:flex-grow">
            <Link
              ...
              smooth={true}
              duration={500}
              activeClass="text-green-500 underline" // Tailwind classes for active state
              className={`block mt-4 md:inline-block md:mt-0 mr-5 cursor-pointer ${
                reachedProjectSection || scrollPosition > 0 ? "text-white" : "text-black"
              }`}
              role="button"
              tabIndex={0}
          ...
          <Link
            ...
            smooth={true}
            duration={500}
            className={`block mt-4 md:mt-0 inline-flex items-center px-3 py-1 text-base border-0 rounded cursor-pointer focus:outline-none ${
              reachedProjectSection || scrollPosition > 0
