...
import "./Navbar.css"; // Ensure this file exists with relevant styles

export default function Navbar() {
    const [scrollPosition, setScrollPosition] = useState(0);
    const [reachedProjectSection, setReachedProjectSection] = useState(false);
    const [isTilted, setIsTilted] = useState(true); // State to manage tilt
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Update tilt state based on scroll position
    ...
    return (
        <header className={`fixed top-0 z-50 w-full ${reachedProjectSection ? 'bg-black' : scrollPosition > 0 ? 'bg-orange-500' : 'bg-gray-200'} text-white backdrop-filter backdrop-blur-lg md:sticky transition-all duration-500`}>
            <div className="container flex flex-col flex-wrap items-center p-5 mx-auto md:flex-row">
                <Link
                    ...
                    smooth={true}
                    duration={500}
                    className={`mb-4 font-bold italic ${reachedProjectSection || scrollPosition > 0 ? 'text-white' : 'text-black'} md:mb-0`}
                ...
                <nav className="flex flex-wrap items-center justify-center text-base md:mr-auto md:ml-4 md:py-1 md:pl-4 md:border-l md:border-yellow-700">
                    <Link
                        ...
                        smooth={true}
                        duration={500}
                        className={`mr-5 cursor-pointer hover:text-green-500 ${reachedProjectSection || scrollPosition > 0 ? 'text-white' : 'text-black'}`}
                        role="button"
                        tabIndex={0}
                    >
                        Projects
                ...
                <Link
                    ...
                    smooth={true}
                    duration={500}
                    className={`inline-flex items-center px-3 py-1 mt-4 text-base border-0 rounded cursor-pointer focus:outline-none md:mt-0 ${reachedProjectSection || scrollPosition > 0 ? 'text-black bg-white hover:bg-gray-300' : 'text-white bg-red-700 hover:bg-red-500'}`}
                    role="button"
                    tabIndex={0}
                >
                    Hire Me
